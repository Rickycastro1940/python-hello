...
- Dataset is **provided, never generated/simulated**. The real file is committed at `data/raw/brasaland_sales.csv` (120 monthly rows, 2016-2025). `src/app.py` (`resolve_dataset_path`) loads the first that exists: `content/contexts/sales-forecasting/brasaland/brasaland_sales.csv` (reference repo) then `data/raw/brasaland_sales.csv` (monorepo). If neither is present it raises `FileNotFoundError` instead of falling back to synthetic data, and the data-dependent tests skip. Columns: `month, revenue_usd, covers_served, avg_ticket_usd, market` (`market` is unused by the model; features are `covers_served, avg_ticket_usd` + engineered `year, month_num`).
